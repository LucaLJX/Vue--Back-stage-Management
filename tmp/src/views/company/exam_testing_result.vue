                ...
                tableData: {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            align: 'center',
                            render (row, column, index) {
                                return `<i-button type="primary" size="small" v-show="${row.status == 3}" v-link="'/company/exam/paperView/' + examId + '/' + ${row.staffPaperInExamId} + '?staffId=' + ${row.staffId}">查看答案</i-button>`
                                    + ` <i-button type="primary" size="small" v-show="${row.status == 3}" v-link="'/company/exam/marking/' + examId + '/' + ${row.staffPaperInExamId} + '?staffId=' + ${row.staffId}">阅卷</i-button>`
                                ;
