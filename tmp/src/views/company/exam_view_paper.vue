    ...
    <div class="lr-right-middle">
        ...
        <div class="common-top-action-container">
            ...
            <div class="common-top-right">
                <i-button type="primary" size="small" v-link="'/company/exam/detail/' + $route.params.id">详情</i-button>
                <i-button type="primary" size="small" v-show="currentEditObj.status == 1" v-link="'/company/exam/edit?id=' + $route.params.id">编辑</i-button>
                <i-button type="primary" size="small" v-show="currentEditObj.status == 1 || currentEditObj.status == 3" @click="generatePaper()">生成试卷</i-button>
                <i-button type="primary" size="small" v-show="currentEditObj.paperCnt && currentEditObj.status == 3" @click="releaseConfirm()" >发布考试</i-button>
                <i-button type="ghost" size="small" v-show="currentEditObj.status == 4" @click="releaseCancelConfirm()" >取消发布</i-button>
            </div>
        </div>
    ...
    export default{
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render (row, column, index) {
                                return `<i-button type="primary" size="small" v-link="'/company/exam/${row.examId}/paperDetail/' + ${row.id}">详情</i-button>`
                                        + ` <i-button type="error" size="small" v-show="currentEditObj.status != 4" @click="tableDataRemove(${index})">删除</i-button>`
        ...
        methods: {
            ...
            tableDataRefresh(fromPageNo1){
                ...
                ezjsUtil.request(apiConstants.exam_viewPaper,
                    {
                        _id: _this.$route.params.id,
