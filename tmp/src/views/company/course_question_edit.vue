...
<template>
    <div class="lr-right-middle">
        ...
        <Card class="z-form-static" dis-hover>
            ...
            <div class="z-form-item">
                <label>章节/知识点:</label>
                <div>{{ modelFormData.editObj.subjectCode | treeCode2LabelFilter}}</div>
            </div>
        </Card>
        <div class="">
            <i-form v-ref:model-form-data-validate :model="modelFormData.editObj" :rules="modelFormData.validate"
                ...
                <Card class="z-card" dis-hover>
                    ...
                    <Form-item label="难易度" prop="difficulty">
                        <i-select :model.sync="modelFormData.editObj.difficulty" placeholder="选择难易度"
                                  class="z-form-controller">
                            <i-option v-for="item in dict.difficulty" :value="item.dictKey">{{ item.dictVal }}
                            </i-option>
                        </i-select>
                    </Form-item>
                    <Form-item label="题干信息" prop="stem">
                        <i-input :value.sync="modelFormData.editObj.stem" placeholder="题干信息"
                    ...
                    <Form-item label="答案" prop="textAnswer"
                        ...
                        <i-input :value.sync="modelFormData.editObj.textAnswer" placeholder="请输入..."
                                 type="textarea" :autosize="{minRows: 5,maxRows: 50}"
                                 class="z-form-controller"></i-input>
                    </Form-item>
                    <Form-item label="答案" prop="judgmentAnswer" v-show="modelFormData.editObj.questionType == 3">
                        <Switch :checked.sync="modelFormData.editObj.judgmentAnswer">
                ...
                <Card class="z-card" dis-hover
                    ...
                    <p slot="title">选项编辑</p>
                    <Form-item
                            v-for="item in modelFormData.editObj.choices"
                            :label="'选项' + ($index + 1)"
                            :prop="'choices.' + $index + '.content'"
                            :rules="{required: true, message: '项目' + ($index + 1) +'不能为空', trigger: 'blur'}">
                        <Row>
                            <i-col span="14">
                                <i-input :value.sync="item.content" placeholder="请输入..."></i-input>
                            </i-col>
                            <i-col span="4" offset="1">
                                <Switch :checked.sync="item.isRight" >
                                    <span slot="open">对</span>
                                    <span slot="close">错</span>
                                </Switch>
                            </i-col>
    ...
    export default{
        ...
        methods: {
            init(){
                ...
                if (id) {
                    ...
                    if (!parentTreeChoosenNode) {
                        return _this.$route.router.go('/company/course');
                    }
                    var subjectId = _this.$parent.treeData.currentValue;
                    var editObj = {
                        id: null,
                        difficulty: 0,
                        judgmentAnswer: false,
            ...
            handleRemoveChoice (item) {
                this.modelFormData.editObj.choices.$remove(item);
            },
                    ...
                    if (valid) {
                        ...
                        _this.$http.post(ezjsUtil.buildUrl(apiConstants.question_edit.url), _this.modelFormData.editObj,
                                ...
                                .then(function (response) {
                                    ...
                                    if(stemId){
                                        _this.$route.router.go('/company/course/question/' + stemId);
                                    }
