  ...
  <div>
    ...
    <div class="z-table-container">
      <Card class="z-form-static" dis-hover v-show="allQuestions[1]">
        <p slot="title">单选题</p>
        <Collapse>
          <Panel v-for="(q, index) in allQuestions[1]" :key="index">
            第{{index + 1}}题(序号{{q.id}})
            <div slot="content">
              <p>{{q.stem}}</p>
              <div class="answer-container">
                <p class="answer-tip">答案:</p>
                <p v-for="(choice, index) in q.choices" :key="index">{{choice.isRight?'对':'错'}} | {{choice.content}}</p>
              </div>
            </div>
          </Panel>
        </Collapse>
      ...
      <Card class="z-form-static" dis-hover v-show="allQuestions[2]">
        <p slot="title">多选题</p>
        <Collapse>
          <Panel v-for="(q, index) in allQuestions[2]" :key="index">
            第{{index + 1}}题(序号{{q.id}})
            <div slot="content">
              <p>{{q.stem}}</p>
              <div class="answer-container">
      ...
      <Card class="z-form-static" dis-hover v-show="allQuestions[3]">
        <p slot="title">判断题</p>
        <Collapse>
          <Panel v-for="(q, index) in allQuestions[3]" :key="index">
            第{{index + 1}}题(序号{{q.id}})
            <div slot="content">
              <p>{{q.stem}}</p>
              <div class="answer-container">
      ...
      <Card class="z-form-static" dis-hover v-show="allQuestions[4]">
        <p slot="title">填空题</p>
        <Collapse>
          <Panel v-for="(q, index) in allQuestions[4]" :key="index">
            第{{index + 1}}题(序号{{q.id}})
            <div slot="content">
              <p>{{q.stem}}</p>
              <div class="answer-container">
      ...
      <Card class="z-form-static" dis-hover v-show="allQuestions[5]">
        <p slot="title">主观题</p>
        <Collapse>
          <Panel v-for="(q, index) in allQuestions[5]" :key="index">
            第{{index + 1}}题(序号{{q.id}})
            <div slot="content">
              <p v-html="q.stem"></p>
              <div class="answer-container">
