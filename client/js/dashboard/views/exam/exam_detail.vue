...
<template>
  <div class="lr-right-middle">
    <div class="history-back-container">
      <i-button type="primary" size="small" v-show="obj.status == 1"
                @click="route2('/company/exam/edit?id=' + $route.params.id)">编辑
      </i-button>
      <i-button type="primary" size="small" @click="paperList()">查看试卷</i-button>
      <i-button type="primary" size="small" @click="releaseConfirm()" v-show="obj.paperCnt && obj.status == 3">发布考试
      </i-button>
    ...
    <Card class="z-form-static" dis-hover>
      ...
      <div class="z-form-item" v-show="obj.startTime">
        <label>开始时间:</label>
        <div>{{ formatDate(obj.startTime)}}</div>
      </div>
      <div class="z-form-item">
        <label>考试时长:</label>
        <div>{{obj.duration}}秒</div>
      </div>
      <div class="z-form-item" v-show="departmentsName">
        <label>参与班级:</label>
        <div v-html="departmentsName"></div>
      </div>
      <div class="z-form-item" v-show="subjectsName">
        <label>考试内容:</label>
        <div v-html="subjectsName"></div>
      </div>
...
//                console.log(arr);
      ...
      paperList(index){
        var _this = this;
        var go = '/company/exam/paperList/' + _this.obj.id;
