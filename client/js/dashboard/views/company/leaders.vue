  ...
  <div>
    ...
    <div class="common-top-container">
      ...
      <div class="common-top-right">
        <i-button class="common-right-btn" type="primary" size="small" @click="modelFormEdit()"
                  icon="ios-personadd-outline">新增管理员
        </i-button>
    ...
    >
      <i-form ref="modelFormDataValidate" :model="modelFormData.editObj" :rules="modelFormData.validate"
              :label-width="80"
              class="z-form">
        <Form-item label="管理员昵称" prop="name">
          <i-input v-model="modelFormData.editObj.name" placeholder="请输入昵称"
                   class="z-form-controller"></i-input>
        </Form-item>
        <Form-item label="登陆账号" prop="num">
          <i-input v-model="modelFormData.editObj.num" placeholder="请输入登陆账号(作为登录账户,初始密码123456)"
                   class="z-form-controller"></i-input>
        </Form-item>
        <Form-item label="角色" prop="role">
          <i-select v-model="modelFormData.editObj.role" placeholder="选择角色"
    ...
    <Modal
      ...
      v-model="grantWithClassModal.visible"
      :mask-closable="false"
      :closable="false"
    >
      <div class="z-vue-tree">
        <Tree ref="grantClassTree" :data="grantWithClassModal.treeData" show-checkbox></Tree>
      </div>
      <div slot="footer">
  ...
  export default {
        ...
        tableData: {
          ...
          columns: [
            ...
            },
            {
              ...
              key: 'role',
              render: (h, params) => {
                return h('div', dict.getVal('leaderRoles', params.row.role));
              }
            },
            {
              title: '操作',
              ...
              render: (h, params) => {
                ...
                var _this = this;
                const modelFormEdit = function () {
                  _this.modelFormEdit(index)
                }
                const grantWithClass = function () {
                  _this.grantWithClass(index)
                }
                ...
                return (
                  <div>
                    <i-button type="primary" size="small" onClick={modelFormEdit} >编辑</i-button>&nbsp;
                    <i-button type="primary" size="small" onClick={grantWithClass} >班级授权</i-button>&nbsp;
                    <i-button type="error" size="small" onClick={tableDataRemove} >删除</i-button>
                  </div>
                )
              }
            },
    ...
    methods: {
      ...
      tableDataRemove (index) {
        var _this = this;
        var msg = '管理员' + '<b> ' + _this.tableData.data[index].name + ' </b>';
        _this.$Modal.confirm({
          title: '确认删除么',
