    ...
    <div class="cr-right-middle">
        ...
        <Card class="z-form-static" dis-hover>
            ...
            <div class="z-form-item">
                <label>班级:</label>
                <div>{{ treeCode2LabelFilter(modelFormData.editObj.departmentCode) }}</div>
            </div>
        </Card>
        <div class="">
            <i-form ref="modelFormDataValidate" :model="modelFormData.editObj" :rules="modelFormData.validate"
                ...
                <Card class="z-card" dis-hover>
                    ...
                    <Form-item label="学号" prop="num" v-show="!modelFormData.editObj.id">
                        <i-input v-model="modelFormData.editObj.num" placeholder="学号"
                                 class="z-form-controller"></i-input>
                    </Form-item>
                    <Form-item label="姓名" prop="name">
                        <i-input v-model="modelFormData.editObj.name" placeholder="姓名"
                                 class="z-form-controller"></i-input>
                    </Form-item>
                    <Form-item label="性别" prop="sex">
                        <i-select v-model="modelFormData.editObj.sex" placeholder="选择性别"
                                  class="z-form-controller">
                            <i-option v-for="item in dict.staffSex" v-model="item.dictKey" :key="item.dictVal">{{ item.dictVal }}
                            </i-option>
                        </i-select>
                    </Form-item>
          ...
          treeCode2LabelFilter(code){
            ...
            init(){
                ...
                if (id) {
                    ...
                    //初始化
                    var parentTreeChoosenNode = _this.$parent.selectedNode.code;
                    if (!parentTreeChoosenNode) {
                        return _this.$router.push('/company/staff');
                    }
                    var editObj = {
                        id: null,
                        name: '',
                        ...
                        departmentId: _this.$parent.selectedNode.id,
                    }
                    ...
                    if (valid) {
                        ...
                        ezjsUtil.request(apiConstants.staff_edit, _this.modelFormData.editObj,
                            function(err, data) {
                                ...
                                var id = data.id;
                                if(id){
                                    _this.$router.push('/company/staff/' + _this.$parent.$data.selectedNode.id + '/staffDetail/' + id);
                                }
                                // _this.modelFormData.loading = false;
                            }
                        );
